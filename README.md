...
저장소에는 Jekyll 사이트 빌드와 GitHub Actions 워크플로우에 필요한 여러 디렉토리와 파일이 포함되어 있습니다. 주요 구성 요소에 대한 간략한 개요는 다음과 같습니다:
...
- `assets`: 이 디렉토리에는 이미지, 스크립트, 스타일시트와 같은 정적 파일들이 포함되어 있습니다.
- `404.html`: 이 파일은 사이트의 사용자 정의 404 오류 페이지를 제공합니다.
- `Gemfile`, `Gemfile.lock`: 이 파일들은 Ruby의 패키지 관리자인 Bundler가 프로젝트의 Ruby 의존성을 추적하는 데 사용됩니다.
- `_config.yml`: 이 파일은 Jekyll 사이트의 주 설정 파일입니다.
- `index.md`: 이 마크다운 파일들은 사이트의 주요 컨텐츠 페이지로 사용됩니다.
